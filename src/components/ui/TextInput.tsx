...
const TextInput: React.FC<TextInputProps> = ({
  ...
  return (
    <div className="relative flex items-center text-gray-400 focus-within:text-white">
      {Icon && <Icon className="size-5 absolute mt-7 ml-3 pointer-events-none transition duration-300 ease-in-out" />}
      <div className="w-full">
        <label htmlFor={labelFor} className="block mb-2 text-sm font-medium text-gray-300 dark:text-white">{label + ":"}</label>
        <input
          id={labelFor}
          name={labelFor}
          type={type}
